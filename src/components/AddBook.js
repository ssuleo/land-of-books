...
function AddBook() {
  ...
  useEffect(() => {
    const fetchCategoriesAndStatuses = async () => {
      ...
      try {
        ...
        const response = await axios.get('http://localhost/land-of-books/backend/addBooks.php');
        console.log(response,"ksdlfm");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost/land-of-books/backend/addBooks.php', formData, {
        headers: {
  ...
  return (
    <Box  w="100vh"ml="250px" paddingTop="40px">
      <form onSubmit={handleSubmit}>
        <VStack spacing={4} align="stretch">
          ...
          <Select id="category" name="category" placeholder="Kategori seçiniz">
